...
![This illustration demonstrates how a regular square mesh can be transformed into several classical topological surfaces by identifying boundary vertices and edges according to different equivalence relations. Each row begins with the same planar quadrilateral mesh and progressively applies edge identifications before arriving at the final embedded surface.
The sphere is obtained by collapsing the entire boundary to a single equivalence class. The cylinder is constructed by identifying one pair of opposite edges while leaving the remaining pair as boundary components. The torus results from identifying both pairs of opposite edges with consistent orientation. The Möbius band is produced by identifying one pair of opposite edges after introducing a half-twist. Finally, the Klein bottle is formed by combining one orientation-preserving and one orientation-reversing edge identification. The illustration emphasizes that these surfaces differ only by the quotient relation imposed on the original mesh rather than by changes to the mesh itself.](media/topo_map_1.PNG)
...
![Construction of classical topological surfaces and their associated dual incidence graphs. The first two figures illustrate how quotient maps transform a regular square mesh into a sphere, cylinder, torus, Möbius band, and Klein bottle through boundary identifications. The latter two figures demonstrate the construction of dual incidence graphs from quadrilateral and hexagonal surface meshes by placing vertices at face centers and connecting neighboring faces sharing an edge. Together, these figures illustrate how topological identifications on the primal mesh induce corresponding global structure within the dual graph while preserving local incidence relationships.](media/dual_4.PNG)
